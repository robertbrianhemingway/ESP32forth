...
#ifndef ENABLE_BME280_SUPPORT
# define OPTIONAL_BME280_SUPPORT
#else
# ifndef SIM_PRINT_ONLY
#  include <Adafruit_Sensor.h>
#  include <Adafruit_BME280.h>
static Adafruit_BME280 *bme = 0;
# endif
# define OPTIONAL_BME280_SUPPORT \
  YV(bme280, BME280Addr, PUSH &bme) \
  YV(bme280, BME280New, bme = new Adafruit_BME280(); DROP) \
  YV(bme280, BME280Delete, delete bme) \
  YV(bme280, BME280Begin, n0 = bme->begin(n0); NIP) \
  YV(bme280, BME280Temperature, n0 = bme->readTemperature()*100) \
  YV(bme280, BME280Pressure, n0 = bme->readPressure()) \
  YV(bme280, BME280Humidity, n0 = bme->readHumidity()*100)
...
vocabulary bme280   bme280 definitions
transfer bme280-builtins
DEFINED? BME280New [IF]
$77 value i2cAddr
: .2places  <# # # [char] . hold #s #> type ; 
: setI2CAddr ( n -- )
  to i2cAddr
;  
: ?Initialized  ( -- addr|0 )
...
: ?temp  ( -- ) 
  ?Initialized
  IF  
    bme280temperature    \ temp*100
    cr ." Temperature (C) = " .2places 
  THEN
;  
: ?press  ( -- ) 
  ?Initialized
  IF  
    bme280pressure    \ pressure*100
    cr ." Pressure (hPa) = " .2places 
  THEN
;  
: ?humid  ( -- ) 
  ?Initialized
  IF  
    bme280Humidity    \ humid*100
    cr ." Humidity (%) = " .2places 
  THEN
;  
: ?BME280All  ( -- )
